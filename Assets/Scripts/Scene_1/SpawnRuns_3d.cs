...
public class SpawnRuns_3d : MonoBehaviour {
    ...
    private void MoveSelecteSlot(GameObject[] columSlots) {
        ...
        for(int i = 0; i < columSlots.Length; i++) {
            ...
            Slot_3d slot = columSlots[i].GetComponentInChildren<Slot_3d>();
                ...
                if(i + 1 < columSlots.Length) {
                    ...
                    //Debug.Log($"МЕТОД  MoveSelecteSlot: ВЫЗВАЛИ ЭВЕНТ ОТМЕНЫ ВЫДЕЛЕНИЯ КЛЕТКИ");
                    slot.DeselectSlot(slot.gameObject);

                    //Debug.Log($"МЕТОД  MoveSelecteSlot: ВЫЗВАЛИ ЭВЕНТЫ ОЧИЩЕНИЯ РУН И ОБЪЕКТОВ = {nextRuns.gameObject}");
                    _selectedAndMeargItem.ClearVariables();
                    _selectedAndMeargItem.ClearSlot();

                    //Debug.Log($"МЕТОД  MoveSelecteSlot: ВЫЗВАЛИ ЭВЕНТ ЗАПОМИНАНИЯ РУНЫ = {nextRuns.gameObject}");
                    ...
                    //MeargGameEvents.onStartEventSetOldObject?.Invoke(currentObject); // Убрать делалось на пробу
                    //Debug.Log($"МЕТОД  MoveSelecteSlot: ВЫЗВАЛИ ЭВЕНТ НАЗНАЧЕНИЯ СТАРОГО ОБЪЕКТА = {nextRuns.gameObject}");

                    //MeargGameEvents.onSetOldSlot?.Invoke(nextSlot);
                    _selectedAndMeargItem.SetOldSlot(nextSlot);
                    //Debug.Log($"МЕТОД  MoveSelecteSlot: ВЫЗВАЛИ ЭВЕНТ НАЗНАЧЕНИЯ СТАРОГО СЛОТА = {nextSlot}");
                    //MeargGameEvents.onSelectedSlot?.Invoke(nextSlot.GetSlotID(), currentObject);

                    //Slot_3d oldSlot = MeargGameEvents.onGetOldSlot?.Invoke();
                    Slot_3d oldSlot = _selectedAndMeargItem.GetOldSlot();
                    //Debug.Log($"МЕТОД  MoveSelecteSlot: СТАРЫЙ СЛОТ = {oldSlot}");
                    //GameObject oldObject = MeargGameEvents.onGetOldObject?.Invoke();
                    GameObject oldObject = _selectedAndMeargItem.GetOldGameObject();
                    //Debug.Log($"МЕТОД  MoveSelecteSlot: СТАРАЯ РУНА = {oldObject}");

                    nextSlot.SelectSlot(nextSlot.gameObject);

                    break;
                } else {
                    //Debug.Log("Последняя клетка в колонке");
                    slot.DeselectSlot(slot.gameObject);

                    _selectedAndMeargItem.ClearVariables();
                    _selectedAndMeargItem.ClearSlot();

                    //Debug.Log("ПЫТАЕМСЯ ОБНУЛИТЬ СТАРЫЙ СЛОТ");
                    break;
                }
            }
        }
        ...
        //    //Debug.Log("МОЖЕТ БЫТЬ МЫ НЕ ЗАПУСКАЕМСЯ");

        //    if(oldSlot.GetSelected() == true) {
        //        Debug.Log($"идентификатор выделенного слота = {oldSlot}");
    ...
    /// <summary>
    /// Устанавливает текущее время таймера для сдвига всех колонок рун
    /// </summary>
